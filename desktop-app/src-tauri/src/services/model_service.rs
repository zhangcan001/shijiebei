...
pub(crate) fn strategy_odds_range(value: f64) -> String {
    bucket(value, &[(0.0, 1.8), (1.8, 2.5), (2.5, 3.51), (3.51, 999.0)], &["1.00-1.79", "1.80-2.49", "2.50-3.50", "3.50+"])
}

pub(crate) fn strategy_probability_range(value: f64) -> String {
    bucket(value, &[(0.0, 0.25), (0.25, 0.35), (0.35, 0.45), (0.45, 0.55), (0.55, 1.01)], &["0%-25%", "25%-35%", "35%-45%", "45%-55%", "55%+"])
}

pub(crate) fn strategy_ev_range(value: f64) -> String {
    bucket(value, &[(-999.0, 0.0), (0.0, 0.05), (0.05, 0.10), (0.10, 0.20), (0.20, 999.0)], &["负EV", "0-5%", "5-10%", "10-20%", "20%+"])
}

pub(crate) fn strategy_advantage_range(value: f64) -> String {
    bucket(value, &[(-999.0, 0.0), (0.0, 0.05), (0.05, 0.10), (0.10, 0.20), (0.20, 999.0)], &["负优势", "0-5%", "5-10%", "10-20%", "20%+"])
}
...
pub(crate) fn strategy_rule_decision(app_dir: &Path, pick: &str, odds: f64, probability: f64, ev: f64, advantage: f64) -> StrategyRuleDecision {
    let dir = training_models_dir(app_dir);
    let Some(rules_payload) = read_json(&dir.join("strategy_rules_v1.json")) else {
        return StrategyRuleDecision { action: "none".to_string(), reason: "未加载训练策略规则".to_string(), matched_rules: Vec::new() };
    };
    let Some(rules) = rules_payload.get("rules").and_then(Value::as_array) else {
        return StrategyRuleDecision { action: "none".to_string(), reason: "训练策略规则为空".to_string(), matched_rules: Vec::new() };
    };
    ...
    let candidates = [
        ...
        ("result_type", if selection == "D" { "draw".to_string() } else { "home_away".to_string() }),
    ];
    ...
    for (dimension, group) in candidates {
        for rule in rules {
            let same_dimension = rule.get("dimension").and_then(Value::as_str).unwrap_or("") == dimension;
            let same_group = rule.get("group").and_then(Value::as_str).unwrap_or("") == group;
            if !same_dimension || !same_group {
                continue;
            }
            let action = rule.get("action").and_then(Value::as_str).unwrap_or("allow_candidate");
    ...
    StrategyRuleDecision {
        action: best_action,
        reason: if reasons.is_empty() { "训练规则未命中明显风险区间".to_string() } else { reasons.join("；") },
        matched_rules: matched,
    }
}

fn model_has_samples(model: &Value) -> bool {
    model.pointer("/training_data_range/train_count").and_then(Value::as_i64)
        .or_else(|| model.pointer("/metrics/train_count").and_then(Value::as_i64))
        .unwrap_or(0) > 0
}

fn feature_value(name: &str, input: &ModelFeatureInput) -> f64 {
    let market_probs = [input.market_home_prob, input.market_draw_prob, input.market_away_prob];
    ...
    match name {
        ...
        "is_home_favorite" => {
            if input.market_home_prob >= input.market_draw_prob && input.market_home_prob >= input.market_away_prob {
                1.0
            } else {
                0.0
            }
        ...
        "is_draw_short" => {
            if input.market_draw_prob >= 0.30 { 1.0 } else { 0.0 }
        }
        ...
        "attack_strength_diff" | "defense_strength_diff" => input.rule_home_lambda - input.rule_away_lambda,
...
fn normalized_feature(model: &Value, feature: &str, input: &ModelFeatureInput) -> f64 {
    let mean = model.pointer(&format!("/scaler_mean/{}", feature))
        .or_else(|| model.pointer(&format!("/normalization/mean/{}", feature)))
        .and_then(Value::as_f64)
        .unwrap_or(0.0);
    let std = model.pointer(&format!("/scaler_scale/{}", feature))
...
fn softmax(scores: &[f64]) -> Vec<f64> {
    ...
    let exps = scores.iter().map(|score| (score - max_score).exp()).collect::<Vec<_>>();
...
fn predict_outcome(model: &Value, input: &ModelFeatureInput) -> Option<(f64, f64, f64)> {
    ...
    let features = model.get("feature_names")
    ...
    for class_name in classes_value.iter().filter_map(Value::as_str) {
        ...
        let mut score = model.pointer(&format!("/intercept/{}", class_name)).and_then(Value::as_f64).unwrap_or(0.0);
        let coeffs = model.pointer(&format!("/coefficients/{}", class_name)).and_then(Value::as_array)?;
    ...
    let find = |target: &str| -> f64 {
        classes.iter().position(|class_name| class_name == target).and_then(|idx| probs.get(idx).copied()).unwrap_or(0.0)
    };
...
fn prediction_outcome_model(dir: &Path) -> Option<Value> {
    ...
    if let Some(ensemble) = ensemble {
        if model_has_samples(&ensemble) {
            let primary = ensemble.get("rust_primary_member").and_then(Value::as_str).unwrap_or("logistic");
            if let Some(member) = ensemble
                .get("members")
                .and_then(Value::as_array)
                .and_then(|members| members.iter().find(|item| item.get("name").and_then(Value::as_str) == Some(primary)))
            {
                let mut member = member.clone();
                member["model_version"] = ensemble.get("model_version").cloned().unwrap_or_else(|| Value::String("outcome_ensemble_model_v1".to_string()));
                member["training_data_range"] = ensemble.get("training_data_range").cloned().unwrap_or(Value::Null);
                return Some(member);
            }
        }
    }
...
fn predict_poisson_lambda(model: &Value, input: &ModelFeatureInput) -> Option<f64> {
    ...
    let mut score = model.get("intercept").and_then(Value::as_f64).unwrap_or(1.2f64.ln());
    for (idx, feature) in features.iter().filter_map(Value::as_str).enumerate() {
        score += coeffs.get(idx).and_then(Value::as_f64).unwrap_or(0.0) * normalized_feature(model, feature, input);
    }
    Some(score.clamp(-2.0, 2.0).exp().clamp(0.2, 4.8))
}

fn handicap_feature_value(name: &str, home_prob: f64, draw_prob: f64, away_prob: f64, home_lambda: f64, away_lambda: f64, line: f64) -> f64 {
    match name {
        "home_prob" => home_prob,
        "draw_prob" => draw_prob,
        "away_prob" => away_prob,
...
pub(crate) fn predict_handicap_with_training_models(app_dir: &Path, input: &ModelFeatureInput, line: f64) -> Option<(f64, f64, f64)> {
    let dir = training_models_dir(app_dir);
    let manifest = read_json(&dir.join("model_manifest.json"))?;
    if !manifest.get("ready").and_then(Value::as_bool).unwrap_or(false) {
        return None;
    }
    ...
    let (home_prob, draw_prob, away_prob) = blend_with_market(read_json(&dir.join("probability_blend_v1.json")).as_ref(), calibrated, input);
    let home_lambda = read_json(&dir.join("goals_home_model_v1.json")).as_ref().and_then(|model| predict_poisson_lambda(model, input)).unwrap_or(input.rule_home_lambda);
    let away_lambda = read_json(&dir.join("goals_away_model_v1.json")).as_ref().and_then(|model| predict_poisson_lambda(model, input)).unwrap_or(input.rule_away_lambda);
    ...
    for class_name in classes_value.iter().filter_map(Value::as_str) {
        classes.push(class_name.to_string());
        let mut score = handicap.pointer(&format!("/intercept/{}", class_name)).and_then(Value::as_f64).unwrap_or(0.0);
        let coeffs = handicap.pointer(&format!("/coefficients/{}", class_name)).and_then(Value::as_array)?;
        for (idx, feature) in features.iter().filter_map(Value::as_str).enumerate() {
            let mean = handicap.pointer(&format!("/scaler_mean/{}", feature)).and_then(Value::as_f64).unwrap_or(0.0);
            let std = handicap.pointer(&format!("/scaler_scale/{}", feature)).and_then(Value::as_f64).unwrap_or(1.0).max(0.0001);
            let value = handicap_feature_value(feature, home_prob, draw_prob, away_prob, home_lambda, away_lambda, line);
            score += coeffs.get(idx).and_then(Value::as_f64).unwrap_or(0.0) * ((value - mean) / std);
        }
        scores.push(score);
    }
    let probs = softmax(&scores);
...
fn calibrate_one(class_bins: Option<&Value>, probability: f64) -> f64 {
    ...
    for bin in bins {
        ...
        if probability >= low && probability < high {
            let empirical = bin.get("empirical_prob").and_then(Value::as_f64).unwrap_or(probability);
...
fn blend_with_market(blend: Option<&Value>, probs: (f64, f64, f64), input: &ModelFeatureInput) -> (f64, f64, f64) {
    let Some(blend) = blend else {
        return probs;
    };
    let mut model_weight = blend.get("model_weight").and_then(Value::as_f64).unwrap_or(1.0).clamp(0.0, 1.0);
    let mut market_weight = blend.get("market_weight").and_then(Value::as_f64).unwrap_or(1.0 - model_weight).clamp(0.0, 1.0);
    let market_sum = input.market_home_prob + input.market_draw_prob + input.market_away_prob;
    if market_sum <= 0.0 {
        return probs;
    }
    let favorite = input.market_home_prob.max(input.market_draw_prob).max(input.market_away_prob);
    if let Some(segment) = blend.get("dynamic_segments")
        .and_then(Value::as_array)
        .and_then(|segments| {
    ...
    {
        model_weight = segment.get("model_weight").and_then(Value::as_f64).unwrap_or(model_weight).clamp(0.0, 1.0);
        market_weight = segment.get("market_weight").and_then(Value::as_f64).unwrap_or(1.0 - model_weight).clamp(0.0, 1.0);
    }
...
pub(crate) fn trained_score_probs(home_lambda: f64, away_lambda: f64, max_goals: u32) -> (Value, Value) {
    let mut scores = Vec::new();
    let mut total = 0.0;
...
pub(crate) fn predict_with_training_models(app_dir: &Path, input: &ModelFeatureInput) -> Option<TrainedPrediction> {
    let dir = training_models_dir(app_dir);
    let manifest = read_json(&dir.join("model_manifest.json"))?;
    if !manifest.get("ready").and_then(Value::as_bool).unwrap_or(false) {
        return None;
    ...
    let calibrated = calibrate_probs(read_json(&dir.join("calibrator_v1.json")).as_ref(), raw_probs);
    let probs = blend_with_market(read_json(&dir.join("probability_blend_v1.json")).as_ref(), calibrated, input);
    let home_lambda = read_json(&dir.join("goals_home_model_v1.json")).as_ref().and_then(|model| predict_poisson_lambda(model, input)).unwrap_or(input.rule_home_lambda);
    let away_lambda = read_json(&dir.join("goals_away_model_v1.json")).as_ref().and_then(|model| predict_poisson_lambda(model, input)).unwrap_or(input.rule_away_lambda);
    let (score_probs_json, total_goals_probs_json) = trained_score_probs(home_lambda, away_lambda, 5);
    Some(TrainedPrediction {
        model_version: outcome.get("model_version").and_then(Value::as_str).unwrap_or("outcome_model_v1").to_string(),
...
pub(crate) fn active_model_info(app_dir: &Path) -> ActiveModelInfo {
    ...
    let Some(manifest) = read_json(&dir.join("model_manifest.json")) else {
        return ActiveModelInfo {
            ...
            fallback_reason: "未检测到 training/models/model_manifest.json，当前使用规则模型。".to_string(),
    ...
    let ready = manifest.get("ready").and_then(Value::as_bool).unwrap_or(false)
        && outcome.pointer("/metrics/train_count").and_then(Value::as_i64).unwrap_or(0) > 0;
    let missing = manifest.get("missing_files").and_then(Value::as_array).map(|items| items.len()).unwrap_or(0);
    let worldcup_status = manifest.pointer("/global_models/worldcup_live_correction_status").unwrap_or(&Value::Null);
    ...
    let worldcup_available = worldcup_status.get("ready").and_then(Value::as_bool).unwrap_or(false)
        || manifest.pointer("/global_models/worldcup_live_correction").and_then(Value::as_str).is_some();
    let strategy_rules_summary = read_json(&dir.join("strategy_rules_v1.json"))
        ...
        .filter_map(|rule| {
            ...
            Some(format!(
                ...
                action,
                rule.get("sample_count").and_then(Value::as_i64).unwrap_or(0),
    ...
    ActiveModelInfo {
        ...
        model_version: if ready {
            manifest.get("active_model_version").and_then(Value::as_str).unwrap_or("outcome_model_v1").to_string()
        } else {
            "rules-dixon-coles-v1".to_string()
        },
        model_type: if ready {
            ensemble.get("model_type")
        ...
        sample_count: metrics.get("sample_count").and_then(Value::as_i64).unwrap_or(0),
        training_data_range: manifest.get("training_data_range").cloned().unwrap_or(Value::Null),
        accuracy: metrics.get("accuracy").and_then(Value::as_f64).unwrap_or(0.0),
        log_loss: metrics.get("log_loss").and_then(Value::as_f64).unwrap_or(0.0),
        brier_score: metrics.get("brier_score").and_then(Value::as_f64).unwrap_or(0.0),
        backtest_roi: backtest.get("roi").and_then(Value::as_f64).unwrap_or(0.0),
        backtest_final_bet_count: backtest.get("final_bet_count").and_then(Value::as_i64).unwrap_or(0),
        backtest_max_drawdown: backtest.get("max_drawdown").and_then(Value::as_f64).unwrap_or(0.0),
        backtest_avg_odds: backtest.get("avg_odds").and_then(Value::as_f64).unwrap_or(0.0),
        backtest_avg_ev: backtest.get("avg_ev").and_then(Value::as_f64).unwrap_or(0.0),
        backtest_warning: backtest.get("warning").and_then(Value::as_str).unwrap_or("").to_string(),
        ...
        global_models_summary: manifest.get("global_models")
            .and_then(Value::as_object)
            .map(|models| {
                let mut rows = vec![
                    format!("胜平负：{}", models.get("outcome").and_then(Value::as_str).unwrap_or("-")),
                    format!("胜平负集成：{}", models.get("outcome_ensemble").and_then(Value::as_str).unwrap_or("-")),
                    format!("概率校准：{}", models.get("calibrator").and_then(Value::as_str).unwrap_or("-")),
                    format!("市场概率融合：{}", models.get("probability_blend").and_then(Value::as_str).unwrap_or("-")),
                    format!("主队进球：{}", models.get("goals_home").and_then(Value::as_str).unwrap_or("-")),
                    format!("客队进球：{}", models.get("goals_away").and_then(Value::as_str).unwrap_or("-")),
                    format!("让球映射：{}", models.get("handicap").and_then(Value::as_str).unwrap_or("-")),
                    format!("世界杯临场修正：{}", models.get("worldcup_live_correction").and_then(Value::as_str).unwrap_or("样本不足")),
                ];
                if let Some(layers) = manifest.get("worldcup_model_v2_layers").and_then(Value::as_array) {
        ...
        worldcup_correction_version: manifest.pointer("/global_models/worldcup_live_correction").and_then(Value::as_str).unwrap_or("").to_string(),
        worldcup_correction_sample_count: worldcup_metrics.get("sample_count").and_then(Value::as_i64).unwrap_or(0),
        worldcup_correction_accuracy: worldcup_metrics.get("accuracy").and_then(Value::as_f64).unwrap_or(0.0),
        worldcup_correction_log_loss: worldcup_metrics.get("log_loss").and_then(Value::as_f64).unwrap_or(0.0),
        worldcup_correction_brier_score: worldcup_metrics.get("brier_score").and_then(Value::as_f64).unwrap_or(0.0),
        worldcup_correction_note: if worldcup_available {
            "已用2018/2022世界杯历史赛前赔率训练，只修正投注推荐置信度，不改写胜平负真实概率。".to_string()
        } else {
            worldcup_report.get("reason").and_then(Value::as_str).unwrap_or("世界杯临场修正层样本不足。").to_string()
        },
    }
}
